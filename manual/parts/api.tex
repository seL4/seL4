...
\newcommand{\threadschedcontextcapdesc}{Capability to the scheduling context that the TCB should run on.}
...
\newcommand{\threadmaxpriodesc}{The threads new maximum priority.}
...
\newcommand{\excepthanddesc}{CPTR to the endpoint which receives IPCs when this thread triggers a temporal fault. This capability is in the caller's cspace.}
...
\inputapidoc{cnode_savetcbcaller}
...
\inputapidoc{tcb_setmaxpriority}
...
\inputapidoc{schedcontext_bindtcb}
\inputapidoc{schedcontext_unbindtcb}
\inputapidoc{schedcontext_bindnotification}
\inputapidoc{schedcontext_unbindnotification}
\ifxeightsix
\clearpage
