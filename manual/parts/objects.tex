...
\begin{description}
    ...
    \item[Scheduling contexts] are an abstraction of CPU execuion time.
    ...
    \item[Reply objects] are used to store single-use reply capabilities,
...
\emph{Wait} is a variant of \emph{Receive} that does not require a reply object to be provided.
...
\begin{description}
    ...
    \item[\apifunc{seL4\_NBWait}{sel4_nbwait}] is used by a thread to poll for
    messages through endpoints or notifications. If no sender or
    ...
    giving the receiver the right to reply to the original sender. The reply
    capability is deposited in a dedicated reply object provided by the receiver, and
    is a single-use right, meaning that the kernel
    ...
    \item[\apifunc{seL4\_ReplyRecv}{sel4_replyrecv}] combines an invocation of the reply capability
    (\apifunc{seL4\_Send}{sel4_send}) and uses the same reply object for the \apifunc{seL4\_Recv}{sel4_recv} phase.
    ...
    \item[\apifunc{seL4\_NBSendWait}{sel4_nbsendwait}] combined an \apifunc{seL4\_NBSend}{sel4_nbsend} and
        \apifunc{seL4\_Wait}{sel4_wait} into one atomic system call.

    \item[\apifunc{seL4\_NBSendRecv}{sel4_nbsendwait}] combined an \apifunc{seL4\_NBSend}{sel4_nbsend} and
        \apifunc{seL4\_Recv}{sel4_recv} into one atomic system call.
    ...
    scheduled with a fresh timeslice, unless it is a sporadically scheduled thread,
    which will be scheduled when the next replenishment is due, see \autoref{sec:scheduling_contexts}.
\end{description}
...
\begin{description}
   ...
   \item[Scheduling contexts] (\obj{SchedulingContext}s; see \autoref{ch:threads}) represent
   ...
   \item[\obj{Reply objects}] (see \autoref{ch:ipc}) track scheduling
...
the size of the resulting objects. Table \ref{tab:obj_sizes} shows the
correspondence between the \texttt{size\_bits} argument ($n$) to
\apifunc{seL4\_Untyped\_Retype}{untyped_retype}, and the resulting
size of each variable-sized object.
        ...
        \begin{center}
            \begin{tabular}{ll}
                ...
                \midrule
                  ...
                  $n$-bit CNode         & $16*2^n$ bytes (where $n \ge 2$) \\
                  Endpoint              & $16$ bytes \\
