...
On the MCS kernel, \emph{Wait} is a variant of \emph{Receive} that does not require a reply object to be provided.
...
\begin{description}
    ...
    \item[\apifunc{seL4\_Wait}{sel4_wait}] (MCS only) is used by a thread to receive
    ...
    \item[\apifunc{seL4\_NBWait}{sel4_nbwait}] (MCS only) is used by a thread to poll for
    messages through endpoints or notifications. If no sender or
    ...
    \item[\apifunc{seL4\_Call}{sel4_call}] combines \apifunc{seL4\_Send}{sel4_send}
      ...
      \apifunc{seL4\_Send}{sel4_send}. What follows next depends on the kernel configuration.
      For MCS configurations, the kernel then updates the \emph{reply object}
      provided by the receiver. A \emph{reply object} is a vessel for tracking
      reply messages, used to to send a reply message and wake up the caller,
      Without MCS, the kernel then deposits a \emph{reply} capability
	  ...
	  For both variants, the calling thread is blocked until a capability to
      the reply object is invoked.
    ...
    \item[\apifunc{seL4\_NBSendWait}{sel4_nbsendwait}] (MCS only) combined an \apifunc{seL4\_NBSend}{sel4_nbsend} and
        \apifunc{seL4\_Wait}{sel4_wait} into one atomic system call.

    \item[\apifunc{seL4\_NBSendRecv}{sel4_nbsendwait}] (MCS only) combined an \apifunc{seL4\_NBSend}{sel4_nbsend} and
    ...
    scheduled with a fresh timeslice. On the MCS kernel this behavior depends on the
    state of the scheduling context, see \autoref{sec:scheduling_contexts}.
\end{description}
