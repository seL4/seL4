...
\begin{description}
    ...
    \item[Scheduling contexts] are an abstraction of CPU execution time.
...
Logically, the kernel provides two system calls, \emph{Send},
\emph{Receive}. However, there are also combinations
...
\begin{description}
    ...
    recent caller only. The \apifunc{seL4\_CNode\_SwapCaller}{cnode_swapcaller}
    and \apifunc{seL4\_CNode\_SwapTCBCaller}{cnode_swaptcbcaller} methods that will be described later can be used to swap the reply
    capability into regular capability space, where it can be used with
    \apifunc{seL4\_Send}{sel4_send} or swapped back in.
    ...
    \item[\apifunc{seL4\_SignalRecv}{sel4_signalrecv}] combines a \apifunc{seL4\_NBSend}{sel4_nbsend} with a \apifunc{seL4\_Recv}{sel4_recv}, and can be called on two different objects (notification objects, reply caps or endpoints).
    Regardless of the success of the non-blocking send, the receive operation will continue. The send operation does 
...
\begin{description}
   ...
   \item[Scheduling contexts] (\obj{SchedulingContext}s; see \autoref{ch:threads}) represent 
       ...
       Initially, there is a capability to \obj{SchedControl}, which 
       allows scheduling context to be populated with parameters, which combined with priority
       control threads access to CPU time. 
