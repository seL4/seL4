...
\apifunc{seL4\_SchedContext\_BindTCB}{schedcontext_bindtcb}, both invocations have the same effect although \apifunc{seL4\_TCB\_Configure}{tcb_configure} allows more thread fields to be set with only one kernel entry. 
...
Threads can be unbound from a scheduling context with \apifunc{seL4\_SchedContext\_UnbindTCB}{schedcontext_unbindtcb}. 
...
Scheduling context parameters can then be set and updated using \apifunc{seL4\_SchedControl\_Configure}, which allows the budget and period to be specified.
...
In addition to explictly binding and removing scheduling contexts through \apifunc{seL4\_SchedContext\_BindTCB}{schedcontext_bindtcb} and \apifunc{seL4\_SchedContext\_UnbindTCB}{schedcontext_unbindtcb}, scheduling contexts can move between threads over IPC.
...
When \apifunc{seL4\_Call}{sel4_call} is used, the generated reply cap ensures that the callee is merely borrowing the scheduling context: when the reply cap is consumed by a reply message being sent, or otherwise deleted, the scheduling context will be returned to the caller.
...
Scheduling contexts can also be bound to notification objects using \apifunc{seL4\_SchedContext\_BindNotification}{schedcontext_bindnotification} and unbound using \apifunc{seL4\_SchedContext\_UnbindNotification}{schedcontext_unbindnotification}.
...
Thread priority (structure \texttt{seL4\_Prio\_t}) consists of two values, priority and maximum controlled priority.
...
Temporal faults handlers can use \apifunc{seL4\_CNode\_SaveTCBCaller}{tcb_cnodesavecaller} to save the servers reply capability and reply with an error to the client, then resetting the server to handle the next client request.
