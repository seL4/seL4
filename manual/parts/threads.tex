...
The kernel switches between domains according to a dynamically configurable,
time-triggered schedule.

Domain support is enabled by setting \texttt{KernelNumDomains} to a value greater than the
default of one at compile time. The maximum number of domain schedule entries is fixed and
equal to \texttt{KernelNumDomainSchedules}.
...
\autoref{ch:bootup:bootinfo}).

The domain schedule is configured with
\apifunc{seL4\_DomainSet\_ScheduleConfigure}{domain_schedule_configure} and
\apifunc{seL4\_DomainSet\_ScheduleSetStart}{domain_schedule_set_start}.
