...
reserved = ('BLOCK', 'BASE', 'FIELD', 'FIELD_HIGH', 'CHERI_CAP', 'MASK', 'PADDING',
            'TAGGED_UNION', 'TAG')
...
def p_fields_field(t):
    ...
    t[0] = t[1] + [(t[3], t[4], False, False)]


def p_fields_field_high(t):
    """fields : fields FIELD_HIGH IDENTIFIER INTLIT"""
    t[0] = t[1] + [(t[3], t[4], True, False)]


def p_fields_padding(t):
    """fields : fields PADDING INTLIT"""
    t[0] = t[1] + [(None, t[3], False, False)]


def p_fields_cheri_cap(t):
    """fields : fields CHERI_CAP IDENTIFIER INTLIT"""
    t[0] = t[1] + [(t[3], t[4], False, True)]
...
#if %(cheri_count)d
    __uintcap_t cheri_words[%(cheri_count)d];
#endif
};
...
cheri_reader_template = \
    """%(inline)s uintptr_t CONST
%(block)s_get_%(field)s(%(block)s_t %(block)s) {
    return %(block)s.cheri_words[%(index)d];
}"""
...
cheri_ptr_reader_template = \
    """%(inline)s uintptr_t PURE
%(block)s_ptr_get_%(field)s(%(block)s_t *%(block)s_ptr) {
    return %(block)s_ptr->cheri_words[%(index)d];
}"""
...
cheri_writer_template = \
    """%(inline)s %(block)s_t CONST
%(block)s_set_%(field)s(%(block)s_t %(block)s, uintptr_t cap) {
    %(block)s.cheri_words[%(index)d] = cap;
    return %(block)s;
}"""
...
cheri_ptr_writer_template = \
    """%(inline)s void
%(block)s_ptr_set_%(field)s(%(block)s_t *%(block)s_ptr, uintptr_t cap) {
    %(block)s_ptr->cheri_words[%(index)d] = cap;
}"""
...
cheri_union_reader_template = \
    """%(inline)s uintptr_t CONST
%(union)s_%(block)s_get_%(field)s(%(union)s_t %(union)s) {
    return %(union)s.cheri_words[%(index)d];
}"""
...
cheri_ptr_union_reader_template = \
    """%(inline)s uintptr_t PURE
%(union)s_%(block)s_ptr_get_%(field)s(%(union)s_t *%(union)s_ptr) {
    return %(union)s_ptr->cheri_words[%(index)d];
}"""
...
cheri_union_writer_template = \
    """%(inline)s %(union)s_t CONST
%(union)s_%(block)s_set_%(field)s(%(union)s_t %(union)s, uintptr_t cheri_cap) {
    %(union)s.cheri_words[%(index)d] = cheri_cap;
    return %(union)s;
}"""
...
cheri_ptr_union_writer_template = \
    """%(inline)s void
%(union)s_%(block)s_ptr_set_%(field)s(%(union)s_t *%(union)s_ptr,
                                      uintptr_t cheri_cap) {

    %(union)s_ptr->cheri_words[%(index)d] = cheri_cap;
}"""
...
class TaggedUnion:
    def __init__(self, name, tagname, tag_slices, classes, tags):
        ...
        # Number of CHERI capabilities in this tagged union. By default it is 0
        # but is incremented if CHERI capabilities are found later on during parsing.
        self.cheri_count = 0
            ...
            if self.sliced_tag:
                if len(value) > 1:
                    ...
                    for i, tag_slice in enumerate(self.tag_slices):
                        _, size, _, is_cheri = ref.field_map[tag_slice]
                        if value[i] > 2 ** size - 1:
    ...
    def resolve(self, params, symtab):
        ...
        for name, value, ref in self.tags:
            if ref.cheri_count:
                self.cheri_count = ref.cheri_count
                break
        ...
        for name, value, ref in self.tags:
            for tag_slice in self.tag_slices:
                _tag_offset, _tag_size, _tag_high, _tag_is_cheri = ref.field_map[tag_slice]
    ...
    def set_base(self, base, base_bits, base_sign_extend, suffix):
        self.base = base
        self.multiple = (self.union_size // base) - self.cheri_count * 2
        ...
        for w in self.tag_offset:
            ...
            if tag_index is None:
                tag_index = (tag_offset - self.cheri_count * CHERI_CLEN) // base

            if ((tag_offset - self.cheri_count * CHERI_CLEN) // base) != tag_index:
                raise ValueError(
    ...
    def generate_hol_proofs(self, params, type_map):
        ...
        for name, value, ref in self.tags:
            ...
            if len(arg_list) == 0:
                ...
                field_eq_list = []
                for field in ref.visible_order:
                    _, size, high, is_cheri = ref.field_map[field]
            ...
            if self.sliced_tag:
                ...
                _, size, _, is_cheri = ref.field_map[self.tagname]
            ...
            for (field, _, size, high, is_cheri) in ref.fields:
                if field in self.tag_slices:
                    continue
    ...
    def generate_hol_defs(self, params):
        ...
        if not self.sliced_tag:
            for name, value, ref in self.tags:
                offset, size, _, is_cheri = ref.field_map[self.tagname]
        ...
        for name, value, ref in self.tags:
            ...
            for field in ref.visible_order:
                offset, size, high, is_cheri = ref.field_map[field]

                if field in self.tag_slices:
                    continue

                if is_cheri:
                    # CHERI caps are always placed from offset 0, so their indices
                    # are just calculated based on the CERI cap length.
                    index = offset // CHERI_CLEN
                else:
                    # For non-CHERI worsd, calculate the "integer" indices of "integer"
                    # words by substracing any CHERI cap sizes; integer words and CHERI
                    # words (and their indices) are different.
                    index = (offset - self.cheri_count * CHERI_CLEN) // self.base
    ...
    def generate(self, params):
        ...
        print(typedef_template %
              {"type": TYPES[options.environment][self.base],
               ...
               "cheri_count": self.cheri_count}, file=output)
        print(file=output)
            ...
            fs = (tag_reader_header_template % subs
                  + "".join([template %
                             dict(subs,
                                  mask=2 ** width - 1,
                                  classmask=self.word_classmask(width),
                                  index=(self.tag_offset[width] -
                                         self.cheri_count * CHERI_CLEN) // self.base,
                                  shift=(self.tag_offset[width] - self.cheri_count * CHERI_CLEN) % self.base)
                             for template, width in zip(templates, self.widths)])
                  + tag_reader_footer_template % subs)
        ...
        for name, value, ref in self.tags:
            ...
            for field in param_fields:
                _, _, _, is_cheri = ref.field_map[field]
                if is_cheri:
                    param_list.append("%s %s" % (TYPES[options.environment][self.base*2], field))
                else:
                    param_list.append("%s %s" % (TYPES[options.environment][self.base], field))
            ...
            for field in ref.visible_order + ([self.tagname] if self.sliced_tag else []):
                ...
                if high:
                    ...
                    fields_cheri.append((field_cheri_index, field))
                    field_cheri_index += 1
                    continue
                else:
                    shift_op = "<<"
            ...
            cheri_inits = [
                "cheri_words[%d] = %s;" % (self.cheri_count - index - 1, name)
                for (index, name) in fields_cheri]

            def mk_inits(prefix):
                return '\n'.join(["    %s%s" % (prefix, word_init) for word_init in word_inits])

            def mk_cheri_inits(prefix):
                return '\n'.join(["    %s%s" % (prefix, word_init) for word_init in cheri_inits])

            print_params = {
                "inline":     INLINE[options.environment],
                "block":      name,
                "union":      self.name,
                "cheri_count": self.cheri_count,
                "gen_params": gen_params,
                "ptr_params": ptr_params,
                "gen_inits":  mk_inits("%s." % self.name),
                "ptr_inits": mk_inits("%s_ptr->" % self.name),
                "gen_cheri_inits":  mk_cheri_inits("%s." % self.name),
                "gen_cheri_ptr_inits":  mk_cheri_inits("%s_ptr->" % self.name),
                "asserts": '  \n'.join(field_asserts)
            }
            ...
            if self.sliced_tag:
                ...
                tagnameoffset, tagnamesize, _, is_cheri = ref.field_map[self.tagname]
                tagindex = self.tag_index
                tagshift = tagnameoffset % self.base
                tagmask = (2 ** tagnamesize) - 1
                tagvalue = self.name + "_" + ref.name

            for field, offset, size, high, is_cheri in ref.fields:
                ...
                if is_cheri:
                    # Reader
                    emit_named("%s_%s_get_%s" % (self.name, ref.name, field),
                               params, cheri_union_reader_template % subs)

                    # Writer
                    emit_named("%s_%s_set_%s" % (self.name, ref.name, field),
                               params, cheri_union_writer_template % subs)

                    # Pointer lifted reader
                    emit_named("%s_%s_ptr_get_%s" % (self.name, ref.name, field),
                               params, cheri_ptr_union_reader_template % subs)

                    # Pointer lifted writer
                    emit_named("%s_%s_ptr_set_%s" % (self.name, ref.name, field),
                               params, cheri_ptr_union_writer_template % subs)
                else:
                    # Reader
                    emit_named("%s_%s_get_%s" % (self.name, ref.name, field),
    ...
    def record_tag_data(self):
        ...
        for tag_slice in self.tag_slices:
            offset, size, _, is_cheri = ref.field_map[tag_slice]
...
class Block:
    def __init__(self, name, fields, visible_order):
        offset = 0
        _fields = []
        self.size = sum(size for _name, size, _high, is_cheri in fields)
        offset = self.size
        self.constant_suffix = ''
        self.cheri_count = 0
        ...
        for _name, _size, _high, _is_cheri in fields:
            offset -= _size
            if not _name is None:
                if visible_order is None:
                    self.visible_order.append(_name)
                _fields.append((_name, offset, _size, _high, _is_cheri))
            if _is_cheri:
                if offset % CHERI_CLEN:
                    raise ValueError("%s Wrong CHERI capability %s alignment of %d. A CHERI capability "
                                     "should be aligned to %d-bit (%d-bytes)" % (name, _name, offset % CHERI_CLEN,
                                                                                 CHERI_CLEN, CHERI_CLEN / 8))
                if _size != CHERI_CLEN:
                    raise ValueError("%s Wrong CHERI capability %s size of %d. A CHERI capability "
                                     "should be %d-bytes" % (name, _name, _size, CHERI_CLEN / 8))
                self.cheri_count += 1

        self.name = name
        self.tagged = False
        self.fields = _fields
        self.field_map = dict((name, (offset, size, high, is_cheri))
                              for name, offset, size, high, is_cheri in _fields)
    ...
    def set_base(self, base, base_bits, base_sign_extend, suffix):
        ...
        self.base_sign_extend = base_sign_extend
        if self.size % base != 0:
            raise ValueError("Size %d of block %s not a multiple of base"
                             % (self.size, self.name))
        self.multiple = self.size // base - (self.cheri_count * 2)
        for name, offset, size, high, is_cheri in self.fields:
            if is_cheri:
                base = CHERI_CLEN
    ...
    def generate_hol_defs(self, params, suppressed_fields=[],
        ...
        if not in_union:
            ...
            for name in self.visible_order:
                offset, size, high, is_cheri = self.field_map[name]
    ...
    def generate_hol_proofs(self, params, type_map):
        ...
        field_eq_list = []
        for field in self.visible_order:
            offset, size, high, is_cheri = self.field_map[field]
    ...
    def generate(self, params):
        ...
        for field in param_fields:
            for _field, _, _, _, is_cheri in self.fields:
                if field == _field:
                    if is_cheri:
                        param_list.append("%s %s" %
                                          (TYPES[options.environment][self.base*2], field))
                    else:
                        param_list.append("%s %s" % (TYPES[options.environment][self.base], field))
        ...
        for field, offset, size, high, is_cheri in self.fields:
            if is_cheri:
                index = offset // CHERI_CLEN
            else:
                index = (offset - self.cheri_count * CHERI_CLEN) // self.base
        ...
        for field, offset, size, high, is_cheri in self.fields:
            ...
            if is_cheri:
                emit_named("%s_get_%s" % (self.name, field), params,
                           cheri_reader_template % subs)

                # Writer
                emit_named("%s_set_%s" % (self.name, field), params,
                           cheri_writer_template % subs)

                # Pointer lifted reader
                emit_named("%s_ptr_get_%s" % (self.name, field), params,
                           cheri_ptr_reader_template % subs)

                # Pointer lifted writer
                emit_named("%s_ptr_set_%s" % (self.name, field), params,
                           cheri_ptr_writer_template % subs)
            else:
                emit_named("%s_get_%s" % (self.name, field), params,
...
if __name__ == '__main__':
    ...
    parser.add_option('--cheri-arch', action='store', default=None,
                      help="If CHERI is enabeled, provide the CHERI architecture here")
    ...
    if options.cheri_arch:
        if "rv32" in options.cheri_arch:
            CHERI_CLEN = 64

        TYPES['sel4'][CHERI_CLEN] = "uintptr_t"
        TYPES['libsel4'][CHERI_CLEN] = "seL4_Register"
