...
/**
 * @xmlonly <manual name="Signal Recv" label="sel4_signalrecv"/> @endxmlonly
 * @brief Perform a signal (non-blocking send with no message) to one capability,
 * then wait on another capability, in one system call
 *
 * @xmlonly
 * See <autoref sec="sys_signalrecv"/>
 * @endxmlonly
 *
 * @param[in] dest The capability to signal.
