...
{
    return (pptr_t)(paddr + KERNEL_PPTR_OFFSET);
}
...
#define pptr_to_paddr(x)   addrFromPPtr((uintptr_t) x)
#define kpptr_to_paddr(x)  addrFromKPPtr((uintptr_t) x)

static inline region_t CONST paddr_to_pptr_reg(const p_region_t p_reg)
{
    region_t pptr_reg = (region_t) {
        .start = paddr_to_pptr(p_reg.start),
        .end   = paddr_to_pptr(p_reg.end)
    };
