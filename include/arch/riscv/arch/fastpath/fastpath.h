...
static inline void NORETURN FORCE_INLINE fastpath_restore(word_t badge, word_t msgInfo, tcb_t *cur_thread)
{
    ...
    void *__capability cur_thread_regs = (void *__capability)cur_thread->tcbArch.tcbContext.registers;
#else
...
#if defined(CONFIG_HAVE_CHERI)
    register void *__capability cur_thread_reg asm(REGN(t0)) = cur_thread_regs;
#else
    register pptr_t cur_thread_reg asm("t0") = cur_thread_regs;
#endif
