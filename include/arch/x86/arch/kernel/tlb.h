...
static inline void invalidatePageStructureCacheASID(paddr_t root, asid_t asid, word_t mask)
{
    ...
    SMP_COND_STATEMENT(doRemoteMaskOp2Arg(
                           IpiRemoteCall_InvalidatePageStructureCacheASID, root, asid, mask));
}
...
static inline void invalidateTranslationSingle(vptr_t vptr, word_t mask)
{
    ...
    SMP_COND_STATEMENT(doRemoteMaskOp1Arg(
                           IpiRemoteCall_InvalidateTranslationSingle, vptr, mask));
}
...
static inline void invalidateTranslationSingleASID(vptr_t vptr, asid_t asid, word_t mask)
{
    ...
    SMP_COND_STATEMENT(doRemoteMaskOp2Arg(
                           IpiRemoteCall_InvalidateTranslationSingleASID, vptr, asid, mask));
}
