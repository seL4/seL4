    ...
    for (i = 0; i < BIT(VTD_PT_BITS); i++) {
        ...
        if (vtd_pte_ptr_get_read(pte)) {
            ...
            if (level == ia32KSnumIOPTLevels - 1) {
                ...
                cte_t *cte = cdtFindAtDepth(capSpaceTypedMemory, (uint32_t)frame, BIT(IA32_4K_bits), (uint32_t)pte, depth);
                assert(cte);
                ...
                cte_t *cte = cdtFindAtDepth(capSpaceTypedMemory, (uint32_t)pt2, BIT(VTD_PT_SIZE_BITS), (uint32_t)pte, depth);
                assert(cte);
...
void unmapVTDContextEntry(vtd_cte_t *vtd_context_slot)
{
    ...
    ptCte = cdtFindAtDepth(capSpaceTypedMemory, VTD_PT_REF(vtd_pt), BIT(VTD_PT_SIZE_BITS), (uint32_t)vtd_context_slot, vtd_cte_ptr_get_cte_depth(vtd_context_slot));
    assert(ptCte);
    /* unmap */
