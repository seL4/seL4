...
compile_assert(kdev_in_kernel_mappings, GET_KPT_INDEX(KDEV_BASE,
                                                      KLVL_FRM_ARM_PT_LVL(1)) == BIT(PT_INDEX_BITS) - 1);
compile_assert(log_buf_in_kernel_mappings, GET_KPT_INDEX(KS_LOG_PPTR,
                                                         KLVL_FRM_ARM_PT_LVL(1)) == BIT(PT_INDEX_BITS) - 1);
 ...
 *
 *  Currently, the memory attributes used in the physical memory window do
...
#ifdef CONFIG_ARM_HYPERVISOR_SUPPORT
/* verify that the physical memory window is at the second entry of the PGD */
compile_assert(physical_window_pgd_location,
               GET_KPT_INDEX(PPTR_BASE, KLVL_FRM_ARM_PT_LVL(0)) == 1);
#else
/* verify that the physical memory window is at the last entry of the PGD */
compile_assert(physical_window_pgd_location,
