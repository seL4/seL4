...
{
    if (boot_state.dev_p_regs.count < sizeof(boot_state.dev_p_regs.list) / sizeof(boot_state.dev_p_regs.list[0])) {
...
try_boot_sys_node(cpu_id_t cpu_id)
{
    ...
    if (!init_sys_state(
                ...
                &boot_state.vesa_info,
                boot_state.mem_regions,
                boot_state.num_drhu,
                boot_state.drhu_list,
...
)
{
    ...
    if ((mbi->flags & MULTIBOOT_INFO_GRAPHICS_FLAG) == 0) {
        boot_state.vesa_info.vbe_mode = -1;
        printf("Multiboot gave us no video information :(\n");
    } else {
        boot_state.vesa_info.vbe_control_info = *mbi->vbe_control_info;
        boot_state.vesa_info.vbe_mode_info = *mbi->vbe_mode_info;
        boot_state.vesa_info.vbe_mode = mbi->vbe_mode;
        printf("Got VBE info in multiboot. Current video mode is %d\n", mbi->vbe_mode);
        boot_state.vesa_info.vbe_interface_seg = mbi->vbe_interface_seg;
        boot_state.vesa_info.vbe_interface_off = mbi->vbe_interface_off;
        boot_state.vesa_info.vbe_interface_len = mbi->vbe_interface_len;
    }
    ...
    if ((mbi->flags & MULTIBOOT_INFO_MEM_MAP) != 0) {
        ...
        while (map < map_end && i < CONFIG_MAX_MEM_REGIONS) {
            if (map->type == MULTIBOOT_MEMORY_AVAILABLE) {
                ...
                boot_state.mem_regions[i].paddr = map->addr;
                boot_state.mem_regions[i].len = map->len;
                i++;
    ...
    while (i < CONFIG_MAX_MEM_REGIONS) {
        boot_state.mem_regions[i].paddr = 0;
        boot_state.mem_regions[i].len = 0;
        i++;
    }
    ...
    boot_state.avail_p_reg.start = MULTIBOOT_HIGHMEM_START;
    ...
    insert_dev_p_reg( (p_region_t) {
        .start = boot_state.avail_p_reg.end, .end = pptr_to_paddr(0)
    });
