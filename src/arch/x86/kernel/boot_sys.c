...
BOOT_CODE static paddr_t load_boot_module(word_t boot_module_start, word_t boot_module_end,
                                          paddr_t load_paddr)
{
    v_region_t v_reg;
    word_t entry;
    ...
    if (!elf_load(elf_file, boot_state.ui_info.pv_offset, v_reg)) {
        printf("ELF image in boot module could not be loaded\n");
        return 0;
    }
