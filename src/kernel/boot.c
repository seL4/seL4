...
BOOT_CODE static word_t calculate_rootserver_size(v_region_t it_v_reg, word_t extra_bi_size_bits)
{
    ...
    size += BIT(seL4_BootInfoFrameSizeBits); // boot info
                                                ...
                                                word_t extra_bi_size_bits)
{
     ...
     * bigger,
     */
    compile_assert(invalid_seL4_BootInfoFrameSizeBits, seL4_BootInfoFrameSizeBits == seL4_PageBits);
    rootserver.boot_info = alloc_rootserver_obj(seL4_BootInfoFrameSizeBits, 1);
                                 ...
                                 vptr_t ipcbuf_vptr, word_t extra_bi_size)
{
    ...
    clearMemory((void *)rootserver.boot_info, seL4_BootInfoFrameSizeBits);
    if (extra_bi_size) {
