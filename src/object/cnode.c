...
cte_t *getReceiveSlots(tcb_t *thread, void *buffer)
{
    cap_transfer_t ct;
    cptr_t cptr;
...
cap_transfer_t PURE loadCapTransfer(void *buffer)
{
    seL4_IPCBuffer *IPCBuffer = buffer;
    return capTransferFromWords(&IPCBuffer->receiveCNode);
}
