...
/* Domain schedules. The duration is in ms for non-MCS and ticks for MCS */
dschedule_t ksDomSchedule[CONFIG_NUM_DOMAIN_SCHEDULES] = {
    DOMAIN_0_MAX_DURATION,
};
...
{
    dschedule_t dom_and_duration = ((uint64_t)domain << 56) | duration;

    ksDomSchedule[index] = dom_and_duration;
}
...
static exception_t decodeDomainScheduleConfigure(word_t length, word_t *buffer)
{
    ...
    if (duration > DOMAIN_0_MAX_DURATION) {
    ...
    if (index == ksDomScheduleStart && duration == 0) {
        userError("Domain Schedule Configure: Starting schedule's duration may not be zero.");
        current_syscall_error.invalidArgumentNumber = 2;
        current_syscall_error.type = seL4_InvalidArgument;
        return EXCEPTION_SYSCALL_ERROR;
    }
...
static exception_t decodeDomainScheduleSetStart(word_t length, word_t *buffer)
{
    ...
    if (ksDomSchedule[index] == DSCHED_END_MARKER) {
        userError("Domain Schedule Set Start: Starting schedule may not be an end marker.");
        current_syscall_error.invalidArgumentNumber = 0;
        current_syscall_error.type = seL4_InvalidArgument;
        return EXCEPTION_SYSCALL_ERROR;
    }
